...
**Notation.** Text in quotation marks followed by **[fetched]** is a real quotation from a page I read. Text in quotation marks followed by **[search result only]** is *the search-result title string as the index returned it* — reproduced exactly as it was returned, ellipses and truncation included, and standing in for a post nobody in this session opened. The two are visually similar and epistemically very different; the grade tag is load-bearing and appears on every single citation below.
...
> **Grade reminder for this section:** every X citation below is **[search result only]** — a search-result title string for a post that could not be opened. The right-hand column of each table names the grade explicitly. The only full-confidence rows are the ones tagged **[fetched]**.
...
> **Grade reminder for this section:** this is the fetched core of the document. §3.3 to §3.6 rest entirely on rule files, agent prompts and slash-command definitions read in full today. The X citations in §3.1 and §3.2 are **[search result only]** and are used to establish that a position exists, not to establish that it is right.
...
> **Grade reminder for this section:** handles are reproduced as they appeared in search-result URLs or in a fetched mirror page. **No profile was opened, no identity was confirmed beyond the handle in the URL, and no engagement figure is reported.** The right-hand column states, per person, what was actually seen.

Who to follow, what they advocate, and how good the evidence actually is.
...
**X posts — [search result only].** For each of these the artefact seen was a search-result listing (URL + index title string) returned today. **The x.com pages themselves were never opened**; no date, author display name, or engagement figure is known for any of them:
