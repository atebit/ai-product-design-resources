...
**Verification note — read this before trusting any line below.** This session's network egress allowed **github.com, raw.githubusercontent.com, gist.github.com and code.claude.com only**. Reddit, Hacker News, arXiv, `survey.stackoverflow.co`, `dora.dev`, `anthropic.com`, `claude.com/blog`, `agents.md`, `playwright.dev`, `storybook.js.org`, `chromatic.com`, `coderabbit.ai`, `docs.cursor.com`, `dev.to`, `infoq.com`, `vercel.com` and every other host tested were refused by the proxy (403 at CONNECT). Consequently: **everything in §4–§9 that carries a GitHub or code.claude.com link was fetched and read today**; everything in §1–§3 and §11 that carries another host's link is a **search-result snippet only — the page did not load and was not independently verified**. Each such claim is marked `[search summary]` inline. A snippet is weaker evidence than this repo normally accepts, and where a number rests only on snippets I say so and do not round it into a conclusion. Verified live September 2026; every claim links its source; anything that could not be fetched is marked.
...
|---|---|---|---|
| [**GitHub Spec Kit**](https://github.com/github/spec-kit) | **136,011★**, MIT, 12,215 forks, 311 open issues; created 21 Aug 2025, pushed **12 Sep 2026**; v1.0.0 released 21 Aug 2026 | Very much alive (contradicting a snippet claiming "no commits for over a month") | `/speckit.analyze` → `/speckit.checklist` → `/speckit.converge`. **`converge` is the only shipped command in any tool that reads the built codebase back against the spec and writes the delta as new tasks** — i.e. it names the miss and files it. |
...
| **AWS Kiro** ([issue tracker](https://github.com/kirodotdev/Kiro), 4.3k★; source not public) | Closed source; GA reported May 2026 `[search summary]` | Active | Specs with contradiction/gap analysis, steering files, hooks, and **correctness testing**: EARS-style requirements compiled into property-based tests run against the generated code `[kiro.dev pages blocked; snippet]`. This is the only mainstream attempt to make the spec *executable against* the implementation rather than merely readable. |
| **Tessl** | Closed; $125M raised; Spec Registry in open beta with 10,000+ specs, free; Framework reported still not GA after ~9 months `[snippet — tessl.io blocked]` | Unverified | A spec registry for third-party libraries, aimed at API hallucination rather than at design misses. Peripheral to this loop. |
...
- Governance: reported as transferred to the Linux Foundation's **Agentic AI Foundation** in late 2025, originally driven by OpenAI with Amp, Google (Jules), Cursor and Factory `[snippet — agents.md blocked]`. The [repo](https://github.com/openai/agents.md) (24.3k★, MIT) is fetched and confirms only that it is "a simple, open format for guiding coding agents"; it publishes **no adoption metrics and no formal spec** — the "spec" is a suggested set of sections (dev environment tips, testing instructions, PR conventions).
...
| **CodeRabbit** | Closed. Org publishes tooling but **no open-source reviewer** — [coderabbitai](https://github.com/coderabbitai) ships `git-worktree-runner` (1.8k★), `awesome-coderabbit` (509★), `skills` (173★) | Org page fetched; product pages blocked | Backend/correctness. |
| **Greptile** | Closed. Reported **$30/developer/month, changed in March 2026 to $30 + $1 per review after 50 reviews** `[search summary]` | Not fetchable | Codebase-indexing reviewer; backend-flavoured. |
| **Cursor Bugbot** | Closed. Reported to have moved from **$40/seat/month to usage-based, ~$1.00–1.50 per run**, from renewals after 8 June 2026 `[search summary]` | Not fetchable | Bug detection; not design. |
...
- "Greptile catches 82% of bugs vs CodeRabbit's 44%, with 11 false positives to CodeRabbit's 2, on 50 real PRs from Sentry, Cal.com and Grafana" `[search summary]`.
- "Cursor Bugbot had a false-positive rate of 4.8%, second-lowest" `[search summary]` — while another summary of the *same* three-week, 146-PR, 679-finding study attributes the **4.8% false-positive rate to CodeRabbit** `[search summary]`.
- "Bugbot posted the highest precision in the field (95.95%) with only 3 false positives" `[search summary]`.
...
| Service | Licence | Price (public, as of today) | Verified |
|---|---|---|---|
| [**Argos**](https://github.com/argos-ci/argos) | **622★, MIT — the only open-source-core option left** | Free to **5,000 screenshots/month**; Pro **$100/month for 35,000**, then **$0.004/screenshot** ($0.0015 for Storybook) `[snippet — argos-ci.com blocked]` | Repo fetched; pricing snippet |
| **Chromatic** | Closed; [chromatic-cli](https://github.com/chromaui/chromatic-cli) 337★ MIT | Free to **5,000 snapshots/month**, no overage on free; paid from **$149–$179/month for 35,000** — *sources disagree on the exact figure* `[search summary]` | CLI repo fetched; pricing snippet |
| **Percy (BrowserStack)** | Closed; [percy/cli](https://github.com/percy/cli) 86★ | Free to 5,000; Professional reported **$199/month for 25,000**; but **BrowserStack has removed Percy from its public pricing page**, with the cheapest advertised tier reported at **$599/month** as of July 2026 `[search summary]` | CLI repo fetched; pricing snippet |
...
**Two findings that change the recommendation in [prototype-review-overlay/05](../prototype-review-overlay/05-feature-landscape-and-mvp-cut.md):** the open-source visual-regression field contracted this year — **Lost Pixel is archived and BackstopJS is seeking a maintainer** — leaving **Argos as the only actively-maintained open-source-core option**, and a very small project at 622★. Meanwhile, **every commercial tier starts at the same free 5,000 screenshots/month**, which for a one-designer prototype practice is almost certainly enough; the trap is the multiplier — *"each viewport and each browser multiplies the count: 200 tests at 2 viewports in 2 browsers is 800 snapshots per build"* `[search summary]`.
...
| **Braintrust** | Closed platform | Free Starter: $10 credits, 1 GB processed data, **10k scores**, 14-day retention, **unlimited users** `[search summary]`; Pro $249/mo | Score-metered and seat-free, which suits a team of one. But 14-day retention is fatal for a *ratchet* — the whole point is a record that outlives the sprint. |
| **LangSmith** | Closed platform | Free Developer: **5k traces, 1 seat**; Plus **$39/seat** `[search summary]` | Trace-first, seat-priced. Weakest fit of the group for a designer. |
...
| 3 | **A `REVIEW.md` with four sections**: what Important means *for a prototype repo*; a nit cap; a skip list (anything CI already enforces); and a verification bar requiring a `file:line` citation. Add a re-review convergence line to stop round seven. | Review noise; rubric drift; endless polish rounds | Free to write; $15–25/review to run the managed service | [code-review docs](https://code.claude.com/docs/en/code-review) |
...
| 8 | **A hosted visual-diff service only when the free 5,000 screenshots/month is exceeded.** Argos first (MIT core, cheapest, agent-ready CLI). | Review UX for diffs across a team | $0 → $100/mo | [Argos](https://github.com/argos-ci/argos) |
...
Every row verified today unless the Price column carries `[search summary]`. Stars and licences from the GitHub repository page or the GitHub API, read 12 September 2026.
...
| AWS Kiro | Spec-first, closed | Closed · price `[search summary]` | EARS requirements compiled into property-based tests | Most interesting idea in the category; unverifiable here |
| Tessl | Spec registry, closed | Closed · registry free `[search summary]` | Library-API hallucination, not design misses | Peripheral |
...
| [Claude Code Review + `REVIEW.md`](https://code.claude.com/docs/en/code-review) | Review agent | Team/Enterprise · **$15–25/review** | **The only tunable review rubric on the market** | **The single most relevant product in this survey** |
...
| CodeRabbit / Greptile / Cursor Bugbot | Review agents | Closed · $30/dev/mo→$1/review; ~$1.00–1.50/run `[search summary]` | Backend correctness | Benchmarks are contradictory (§6.3); not design-relevant |
...
| [Argos](https://github.com/argos-ci/argos) | Visual regression | **MIT** · 622★ · free ≤5,000/mo, $100/mo/35k `[search summary]` | Deterministic visual gate + review UI | **Only maintained OSS-core option** |
| Chromatic | Visual regression | Closed · free ≤5,000/mo, $149–179/mo `[search summary]` | Storybook-native gate | Strong if you already run Storybook |
| Percy | Visual regression | Closed · pricing page withdrawn `[search summary]` | Gate | **Avoid new adoption** |
...
| Braintrust | Eval platform | Closed · free 10k scores, **14-day retention** `[search summary]` | Scoring platform | Retention kills the ratchet |
| LangSmith | Eval platform | Closed · $39/seat `[search summary]` | Tracing platform | Weakest fit for a designer |
...
5. **Argos** — [github.com/argos-ci/argos](https://github.com/argos-ci/argos) · **622★ · MIT**. *Why:* after Lost Pixel's archival and BackstopJS's maintainer notice, **the only actively-maintained open-source-core visual-regression platform**; free to 5,000 screenshots/month; captures locally so you control the count. *Caveats:* small project (622★); pricing verified by snippet only; hosted plan is closed even though the core is MIT.
...
### Search-result snippets only — page blocked by this session's egress proxy, not independently verified
