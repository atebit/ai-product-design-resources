...
| **State coverage** (empty / loading / error present) | No standard tool. Practitioner pattern: ui-craft's eight-state lattice — idle, loading, empty, error, partial, **success**, conflict, offline — of which the seven non-happy states are the `/unhappy` "state-first pass" ([state-design.md](https://raw.githubusercontent.com/educlopez/ui-craft/main/skills/ui-craft/references/state-design.md), re-verified 12 September 2026; this doc originally described the lattice as seven-state, omitting Success — see [iteration-repair-and-rubrics/04 §3](../iteration-repair-and-rubrics/04-design-craft-as-checkable-criteria.md)), plus a deterministic 0–100 UICraftScore combining 43 anti-slop rules, token discipline (raw hex, off-scale spacing) and five static a11y checks ([ui-craft](https://github.com/educlopez/ui-craft), MIT, 325 stars as of 12 September 2026) | Whether a story/route/variant *exists* per state when states are enumerated in the construction file or Storybook | Whether the state is *designed* rather than stubbed — a judge or human call |
...
| State coverage | Deterministic (presence) + judge (quality) | Enumerated states in construction file / stories; ui-craft eight-state lattice | Presence exact; quality tentative | Gate (presence) + score (quality) |
...
| ui-craft | https://github.com/educlopez/ui-craft | Agent skill: eight-state lattice with a seven-state `/unhappy` pass, deterministic UICraftScore, Nielsen 1–5 UsabilityScore; MIT, 325 stars | fetched OK | skills |
