...
A 19-doc series in [docs/research/prototype-construction](../prototype-construction/00-architecture-synthesis.md) investigates an original architecture: codify the design system as primitives, have the LLM emit a small schema-validated construction file (JSON/YAML) instead of raw code, expand it deterministically with a builder skill, and iterate via surgical patches. Verdict: sound and well-precedented (server-driven UI, Mitosis, Vercel json-render converge on the same insight), with estimated ~5–10× token savings per screen and ~30–50× on iteration — but it only pays off when >60–70% of screens are expressible in the pattern library. A September-2026 deep-dive pass closed every remaining open question (behavior/interactivity encoding, schema migration, catalog extraction, responsive/motion, spec-authorship UX, pattern harvesting) and re-verified the whole thesis live: the core bet is confirmed but half-commoditized — Google's A2UI and Vercel's json-render (1.36M weekly downloads) now ship catalog-constrained, schema-validated, patch-iterated generation, so the plan is to reuse their catalog format and runtime and build only the four pieces nobody ships: deterministic offline codegen, the pattern layer, plan/apply preview, and the catalog extractor. See the [architecture synthesis](../prototype-construction/00-architecture-synthesis.md) for the pipeline, decision points, and a falsifiable experiment roadmap (E0–E7).
...
The [prototype-construction series](../prototype-construction/00-architecture-synthesis.md) is past the research stage: it has a concrete pipeline design and an ordered experiment roadmap (E0 baseline → E1 vertical slice → granularity/format/spec ablations → E7 catalog migration), revised after the 2026 landscape refresh to build E1 on top of Vercel's json-render catalog format rather than a bespoke schema. Building E0+E1 would produce several of this repo's flagship artifacts in one stroke: a catalog-codification guide with extractor scripts (now grounded in an 11-design-system extraction audit), a builder skill (SKILL.md + scripts), an intent template (revised for how designers actually author intent — not raw YAML), a migration-module format, and iteration hooks — concrete instances of Tier 1 items 1, 3, and 5.
